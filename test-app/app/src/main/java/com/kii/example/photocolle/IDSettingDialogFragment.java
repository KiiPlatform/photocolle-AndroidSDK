...
public class IDSettingDialogFragment extends DialogFragment
    implements OnClickListener, OnDismissListener
{
    ...
    public IDSettingDialogFragment() {
        super();
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        TimePicker timePicker = (TimePicker)dialog.findViewById(
                R.id.id_setting_time_Picker);
        timePicker.setIs24HourView(true);

        EditText maxResultsEdit = (EditText)dialog.findViewById(
                R.id.id_setting_maxresults_edit);
        if (maxResults == null) {
        ...
        Spinner sortSpinner = (Spinner)dialog.findViewById(
                R.id.id_setting_sort_spinner);
        sortSpinner.setSelection(sortType.getNumber() - 1);

        View button = dialog.findViewById(R.id.id_setting_button);
    ...
    private static Integer getMaxResults(Dialog dialog) {
        EditText edit = (EditText)dialog.findViewById(R.id.id_setting_maxresults_edit);
    ...
    private static Integer getStart(Dialog dialog) {
        EditText edit = (EditText)dialog.findViewById(R.id.id_setting_start_edit);
        String text = edit.getText().toString();
        if (text.length() == 0) {
            return null;
        }
